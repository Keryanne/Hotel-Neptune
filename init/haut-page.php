<!doctype html>
<html>
    <!--Début du head -->
    <head>
        <meta charset="utf-8" />
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content='width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0'> <base href="https://www.hotelvistabella.com/"> 
        <link type="image/ico" href="https://www.hotel-neptune.fr/images/favicon.png" rel="shortcut icon">
        <title>Hôtel Neptune</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-giJF6kkoqNQ00vy+HMDP7azOuL0xtbfIcaT9wjKHr8RbDVddVHyTfAAsrekwKmP1" crossorigin="anonymous">
        <link rel="stylesheet" type="text/css" href="css/styles.min.css">
        <link rel="stylesheet" type="text/css" href="css/overrides.css">
        <link rel="stylesheet" href="">
        
        <script>
            (function(h,o,t,j,a,r){
                h.hj=h.hj||function(){(h.hj.q=h.hj.q||[]).push(arguments)};
                h._hjSettings={hjid:47977,hjsv:5};
                a=o.getElementsByTagName('head')[0];
                r=o.createElement('script');r.async=1;
                r.src=t+h._hjSettings.hjid+j+h._hjSettings.hjsv;
                a.appendChild(r);
            })(window,document,'//static.hotjar.com/c/hotjar-','.js?sv=');
        </script>
    </head>
        <!-- Fin du Head-->

        <!-- Début du Body -->
    <body data-page="home">

        <!-- Début de la barre de nav -->
        <header>
            <div id="header-absolute">
                <div class="container">
                    <a class="logo" href="./index.php">
                        <img src="https://www.hotel-neptune.fr/images/logos/theoriginals_s.png" alt="Hôtel Neptune à Carnon" style="float:left;width:200px;height:40px;margin-left: -50px;">
                    </a>
                    <nav id="main-menu">
                        <ul class="sf-menu">                    
                            <li  data-element-id="1">
                                <a href="<?php echo RACINE_SITE; ?>index.php">
                                    L'Hôtel
                                </a>
                                <ul>                
                                    <li  data-element-id="1.1">
                                        <a href="https://www.hotel-neptune.fr/">
                                        Bienvenue
                                        </a>
                                    </li>
                                    <li  data-element-id="1.3">
                                        <a href="https://www.hotel-neptune.fr/hotel-the-originals-montpellier/photos-le-neptune.php">
                                        Photos & Vidéos    
                                        </a>
                                    </li>
                                    <li  data-element-id="1.2">
                                        <a href="https://www.hotel-neptune.fr/hotel-montpellier-plage/tarif-reservation.php">
                                            Tarifs & Services
                                        </a>
                                    </li>    
                                </ul>
                            </li>                            
                            <li  data-element-id="2">
                                <a href="<?php echo RACINE_SITE; ?>chambres.php">
                                Chambres
                                </a>
                            </li>                            
                                                     
                        </ul>
                    </li>                            
                    <li  data-element-id="6">
                        <a href="https://www.hotel-neptune.fr/sejour-montpellier-plage/week-end-carnon.php">
                            Offres
                        </a>
                                </li>                            
                            <li  data-element-id="8">
                                <a href="https://www.hotel-neptune.fr/">
                                    Réservations
                                </a>
                            </li>
                        </ul>
                    </nav>
                </div>
                <a href="#main-menu-mobile" id="hamburger">
                    <span></span>
                </a>
            </div>
            <!-- Fin de la configuration de la barre de navigation pour pc-->
            <!-- Début de la configuration de la barre de navigation pour mobile-->
            <nav id="main-menu-mobile">
                <ul class="sf-menu">                    
                    <li  data-element-id="1">
                        <a href="<?php echo RACINE_SITE; ?>index.php">
                            L'Hôtel
                        </a>
                        <ul>                
                            <li  data-element-id="1.1">
                                <a href="https://www.hotel-neptune.fr/">
                                Bienvenue
                                </a>
                            </li>
                            <li  data-element-id="1.2">
                                <a href="https://www.google.fr/maps/place/H%C3%B4tel+Neptune/@43.545289,3.973821,17z/data=!3m1!4b1!4m2!3m1!1s0x12b6bb3f6f499f5b:0xa4273d08bd579ecb?hl=fr">
                                Localisation
                                </a>
                            </li>
                            <li  data-element-id="1.3">
                                <a href="https://www.hotel-neptune.fr/hotel-the-originals-montpellier/photos-le-neptune.php">
                                Photos & Vidéos    
                                </a>
                            </li>
                            <li  data-element-id="1.2">
                                <a href="https://www.hotel-neptune.fr/hotel-montpellier-plage/tarif-reservation.php">
                                    Tarifs & Services
                                </a>
                            </li>    
                        </ul>
                    </li>                            
                    <li  data-element-id="2">
                        <a href="<?php echo RACINE_SITE; ?>chambres.php">
                        Chambres
                        </a>
                    </li>                            
                    <li  data-element-id="3">
                        <a href="https://www.letrident-restaurant.com/">
                        Restaurants
                        </a>
                        <ul>                      
                            <li  data-element-id="3.408">
                                <a href="https://www.letrident-restaurant.com/">
                                Le Trident    
                                </a>
                            </li>
                        </ul>
                    </li>                                                                  
                    <li  data-element-id="5">
                        <a href="https://www.hotel-neptune.fr/salle-arena-montpellier/concerts-evenements.php">
                            Evénements    
                        </a>
                    </li>                            
                    <li  data-element-id="7">
                        <a href="https://www.hotel-neptune.fr/salle-arena-montpellier/concerts-evenements.php">
                            Environnement    
                        </a>
                        <ul>             
                    <li  data-element-id="7.1">
                        <a href="https://www.hotel-neptune.fr/sports-montpellier/loisirs-herault.php">
                            Sports & Loisirs    
                        </a>
                    </li>
                    <li  data-element-id="7.3">
                        <a href="https://www.hotel-neptune.fr/seminaire-montpellier/location-salle.php">
                            Groupes & Séminaires    
                        </a>
                    </li>
                    <li  data-element-id="7.2">
                        <a href="https://www.hotel-neptune.fr/palavas-grande-motte/sejour-herault.php">
                            Visites de la région    
                        </a>
                    </li>
                    <li  data-element-id="7.1001">
                        <a href="https://www.hdmedia.fr/visite-virtuelle/hd/cbpldpvif-neptune.html">
                            Visite Virtuelle
                        </a>
                    </li>
                </ul>
            </li>                            
            <li  data-element-id="6">
                <a href="https://www.hotel-neptune.fr/sejour-montpellier-plage/week-end-carnon.php">
                    Offres
                </a>
                        </li>                            
                    <li  data-element-id="8">
                        <a href="https://www.hotel-neptune.fr/">
                            Réservations
                        </a>
                    </li>
                </ul>
            </nav>
            <!-- Fin de la barre de navigation mobile-->
            <a id="book-link" href="">
                Réservez dès maintenant 
                <span class="rho-icon-right"></span>
            </a>
                <!-- Début du défilement d'images en dessous de la barre de navigation -->

            <div id="header-slider" data-page="home" class="carousel slide" data-ride="carousel">
                <div class="carousel-inner" role="listbox">
                    <div class="item active">
                        <div class="slider-image">
                            <img src="https://www.hotel-neptune.fr/images/anim/ph-1.jpg?v" alt="" title="" >
                        </div>
                    </div>
                    <div class="item ">
                        <div class="slider-image">
                            <img src="https://www.hotel-neptune.fr/images/anim/ph-3.jpg?v" alt="" title="" >
                        </div>
                    </div>
                    <div class="item ">
                        <div class="slider-image">
                            <img src="https://www.hotel-neptune.fr/images/anim/ph-6.jpg?v" alt="" title="" >
                    </div>
                </div>
                <div class="item ">
                    <div class="slider-image">
                        <img src="https://www.hotel-neptune.fr/images/anim/ph-2.jpg?v" alt="" title="" >
                    </div>
                </div>
                <div class="item ">
                    <div class="slider-image">
                        <img src="https://www.hotel-neptune.fr/images/anim/ph-4.jpg?v" alt="" title="" >
                    </div>
                </div>
            </div>
        </div>
    </header>